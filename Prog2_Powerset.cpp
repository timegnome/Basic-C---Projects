...
void checkSet(int *stack, int k, int *bestSet, int &bestSize, float *A){
	...
	if (k < 2) {
		...
		for (i = 0; i<k - 1; i++) {
			if (A[stack[i + 1] - 1] >= A[stack[i + 2] - 1])							// decrease each index by one since the indices of array A are in     	
				return;																// the range 0..n-1 and not 1..n 	
																					// WRITE CODE (AN IF STATEMENT) TO CHECK THAT THE ELEMENTS IN ARRAY
																					// A AT INDICES stack[i+1]-1 AND stack[i+2]-1 ARE IN increasing ORDER
